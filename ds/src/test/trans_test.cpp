...
void trans_test::convert_sff_netlist(const std::string& name) {
	...
	if (scan_nl !=0){
		...
		ds_trans::write_test_procedures(name + "_dofile", name + "_test_proc", name + "_scan_group", sm, tp_slow, tp_fast, properties, name + "_patterns.wgl", name + "_faults", name + "_patterns.v");
		delete scan_nl;
	}
